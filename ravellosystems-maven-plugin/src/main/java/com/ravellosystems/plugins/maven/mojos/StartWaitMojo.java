...
public class StartWaitMojo extends ApplicationMojo {
	...
	public void execute() throws MojoExecutionException, MojoFailureException {
		try {
			...
			ApplicationService service = RavelloRestFactory.get(new CredentialsImpl()).application();
			Application application = service.findApplication(applicationName);
			getLog().info("ravello: start");
			boolean stoped = service.checkState(application.getId(), Application.STATE.STARTED);
			if (!stoped) {
				getLog().info("ravello: Application " + application.getName() + " was started already.");
				return;
			}
			service.start(application.getId(), autoStop);
			getLog().info("ravello: await");
			service.awaitForApplicationState(application.getId(), timeout, Application.STATE.STARTED);
